  ...
  const renderGenreItem = ({item, index}) => {
    return (
      <GenreGrid
        ...
        title={item.title}
        onSelect={() => {
          navigation.navigate('SongsListScreen', {gid: item.id}); //passing the id of genre as params to view its songs in the next screens
        }}
      />
    );
  };
  ...
  const renderSongItem = ({item, index}) => {
    return (
      <Recomm
        ...
        artist={item.artist}
        onSelect={() =>
          navigation.navigate('SongsPlayScreen', {
            sid: item.id,
            gid: item.genre,
          })
        }
  ...
  return (
    <View style={{backgroundColor: 'black', padding: 10}}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.subHeader}>Choose genre</Text>
